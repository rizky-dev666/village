    ...
    const fetchData = async () => {
      try {
        ...
        const formatted = res.data.map((item) => ({
          id: item.id_wisata,
          title: item.judul_wisata,
          description: item.isi_wisata,
          image: item.gambar_wisata,
          penulis: item.penulis_wisata || "Administrator",
        }));
  ...
  return (
    <div className="min-h-screen py-10 bg-[#f7f7f7]">
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        <h2 className="text-xl sm:text-2xl font-bold text-green-700 mb-1">Destinasi Wisata</h2>
        <p className="mb-6 text-gray-600 text-sm sm:text-base">
          Beberapa tempat destinasi wisata yang ada di Desa Cikupa
        </p>
