...
export default function BumdesSection() {
    ...
    const fetchData = async () => {
      try {
        ...
        const formatted = response.data.map((item) => ({
          ...
          image: item.foto_usaha,
        }));

        setData(formatted.reverse()); // tampilkan data terbaru di atas
      } catch (error) {
        console.error("Gagal mengambil data BUMDes:", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <section id="bumdes" className="py-10">
      <div className="mb-8">
        <h2 className="text-xl sm:text-4xl font-bold text-green-700 mb-1">BUMDes</h2>
